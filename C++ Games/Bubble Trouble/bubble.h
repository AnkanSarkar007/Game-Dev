...
/* Bubble Vars */
const int BUBBLE_START_X = 1000;
const int BUBBLE_START_Y = 500;
const int BUBBLE_DEFAULT_RADIUS = 25;
const int BUBBLE_RADIUS_THRESHOLD = 25;
int BUBBLE_DEFAULT_VX = 500;
int BUBBLE_DEFAULT_VY = 500;
int gravity = 25;
...
public:
    Bubble(double cx, double cy, double r, double vx_=BUBBLE_DEFAULT_VX, double vy_=BUBBLE_DEFAULT_VY, Color color_=COLOR(255, 0, 0))
    {
        // Bubble constructor
        color = color_;
    ...
    void nextStep(double t)
    {
        ...
        if ((vy > 0 && new_y > (WINDOW_Y - 50 - circle.getRadius())) // bounce along Y direction at bottom border
            ||
            (vy < 0 && new_y < (25 + circle.getRadius()))) // bounce along Y direction at top border
        {
        ...
        vy += gravity; // this incorporates the parabolic motion
